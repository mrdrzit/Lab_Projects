...
macro "AutoProcessing v6.0.2 ANSI [Y]" {
print("\\Clear");
...
//list = split(contents, "\n");
  ...
  for (i = 0; i < list_file_names.length; i++) { //Loop para selecionar apenas imagens .zvi
    if(File.isDirectory(dir + File.separator + list_file_names[i]))
      exit("Você precisa selecionar dentro da pasta onde estÃ£o os arquivos");
...
for ( i=0; i < qtd; i++ ) {
  ...
  //Retira o background
  //Aqui apenas o filtro do BRDU não está com um rolling window de 30px
  ...
  //Faz o composite com as as imagens sem background
  ...
  //Seleciona todas as imagens e salva o composite como um .TIF para análise
  run("Show All");
