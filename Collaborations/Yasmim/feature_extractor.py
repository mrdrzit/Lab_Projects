...
raw_position_data = os.path.join(script_dir, 'raw_data')
raw_features_data = os.path.join(script_dir, 'raw_extracted_features')

# Check if data folders exist
if not os.path.exists(raw_position_data):
    logging.error(f"Missing data directory: {raw_position_data}")
    raise FileNotFoundError(f"Missing data directory: {raw_position_data}")

if not os.path.exists(raw_features_data):
    logging.info(f"Creating missing directory: {raw_features_data}")
    os.makedirs(raw_features_data)

# Load DLC tracking data
tracking_data_files = [os.path.join(raw_position_data, file) for file in os.listdir(raw_position_data) if file.endswith('.h5') if "skeleton" not in file]
skeleton_tracking_data_files = [os.path.join(raw_position_data, file) for file in os.listdir(raw_position_data) if file.endswith('.h5') if "skeleton" in file]

logging.info(f"Tracking data file found: {len(tracking_data_files)}")
logging.info(f"Skeleton tracking data file found: {len(skeleton_tracking_data_files)}")

for tracking_data_file, skeleton_tracking_data_file in zip(tracking_data_files, skeleton_tracking_data_files):
    ...
    # Load the skeleton tracking data
    try:
        skeleton_tracking_data = pd.read_hdf(skeleton_tracking_data_file)
        logging.info(f"Successfully loaded skeleton tracking data")
    except Exception as e:
        logging.error(f"Error loading skeleton tracking data: {e}")
        raise
    ...
    try:
        ...
        sniffing_file_path = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), 'sniffing_features')), f'{current_animal_name}.csv')
        resting_file_path = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), 'resting_locomotion_features')), f'{current_animal_name}.csv')
        locomotion_file_path = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), 'resting_locomotion_features')), f'{current_animal_name}.csv')
        grooming_file_path = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), 'grooming_features')), f'{current_animal_name}.csv')
        rearing_file_path = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), 'rearing_features')), f'{current_animal_name}.csv')
